...
function tvspf = threat_field_args(x, y, t, field_name, op_pointwise, field_params)

if strcmp(field_name, 'peaks')
% 	load tvspf_parameters_peaks.mat t_final n_peaks coeff_peaks_0 coeff_peaks_rate
    min_assigned = field_params.min_assigned;
    max_assigned = field_params.max_assigned;
    t_final = field_params.t_final;
    n_peaks = field_params.n_peaks;
    coeff_peaks_0 = field_params.coeff_peaks_0;
    coeff_peaks_rate = field_params.coeff_peaks_rate;
elseif strcmp(field_name, 'map')
    load map_q1632.mat Img;
elseif strcmp(field_name, 'poly')
	load tvspf_parameters_poly.mat t_final n_poly coeff_poly_0 coeff_poly_rate wksp
end
...
if		strcmp(field_name, 'peaks')
	...
	F_max	= 0;
	for m01 = 1:numel(t)
		Fxy(:, :, m01)		= Fxy(:, :, m01) + 0;					% +1 to ensure strictly positive
		
		dt_Fxy(:,:,m01)		= dt_Fxy(:, :, m01);
        dt2_Fxy(:,:,m01)	= dt2_Fxy(:, :, m01);
...
elseif	strcmp(field_name, 'map')
	...
	%********** GET IN THE RIGHT FORMAT AS THE OTHER TWO **********
	%********** INCLUDE SHIFTING *************
...
% --------- Scale and store in Struct ----------------------------
F_max = max(Fxy(:)); % This doesn't seem to scale the field to a max value? Fxy/max(Fxy(:))?
F_min = min(Fxy(:));
tvspf.field = (max_assigned - min_assigned)*(Fxy - F_min)/...
              (F_max - F_min) + min_assigned; % changed from Fxy/F_max
if strcmp(field_name,'peaks')
    tvspf.difft		= (max_assigned - min_assigned)/(F_max - F_min)*dt_Fxy;
    tvspf.difft2	= (max_assigned - min_assigned)/(F_max - F_min)*dt2_Fxy;
else
    tvspf.difft	= dt_Fxy;
end
tvspf.min	= min_assigned;
tvspf.max	= max_assigned;
tvspf.n_tlr = n_tlr;
% ---------------------------------------------------------------
...
function tvspf_pt = threat_field_pt(x_pt, y_pt, t_pt)
	if strcmp(field_name, 'peaks')
		...
		for m011 = 1:n_peaks
			...
			dx_Fxyt_pt	= dx_Fxyt_pt - ( (x_pt - coeff_peaks_t_pt(2, m011)) ...
				./ (coeff_peaks_t_pt(4, m011)^2) ) * coeff_peaks_t_pt(1, m011) * Fxytm_pt;
			dy_Fxyt_pt	= dy_Fxyt_pt - ( (y_pt - coeff_peaks_t_pt(3, m011)) ...
				./ (coeff_peaks_t_pt(5, m011)^2) ) * coeff_peaks_t_pt(1, m011) * Fxytm_pt;
		end

		tvspf_pt.field  = 1 + Fxyt_pt;
		tvspf_pt.difft	= dt_Fxyt_pt;
